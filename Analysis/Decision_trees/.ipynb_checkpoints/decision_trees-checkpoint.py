...
parameters = {'max_depth':range(2,100),'min_samples_leaf':range(2,100), 
              'criterion' :['gini', 'entropy']}

clf = GridSearchCV(DecisionTreeClassifier(random_state = 8), 
                   parameters, n_jobs=25)
...
parameters = {'max_features':['sqrt'], 'n_estimators':[50, 100, 250, 500, 1000, 2000],
              'max_depth':range(2,100),'min_samples_leaf':range(2,100), 
              'criterion' :['gini', 'entropy']}
...
importances_1 = [x for x in importances if x > 0.05]
importances_1
