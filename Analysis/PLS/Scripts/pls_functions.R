...
StabilityPlot_sgPLS=function(X, Y, Xgroups, NGroup, NIter=100){
  MyStab=NULL
  
  pb=txtProgressBar(style=3)
  for (alpha in seq(0.99,0.01,by=-0.01)){
    setTxtProgressBar(pb, 1-(alpha/0.01-1)/(99))
    TmpStab=NULL
    for (k in 1:NIter){
      s=sample(seq(1, nrow(X)), size=0.8*nrow(X), replace = FALSE) # subsampling procedure
      X_sub=X[s,]
      ...
      TmpsPLS=sgPLSda(as.matrix(X_sub), Y_sub, ncomp = 1, ind.block.x = Xgroups, keepX = NGroup, alpha.x = alpha)
      TmpStab=rbind(TmpStab, TmpsPLS$loadings$X[,1])
    }
    MyStab=rbind(MyStab, (apply(TmpStab, 2, FUN=function(x){sum(x!=0)})/NIter))
  }
  
  rownames(MyStab)=round(seq(0.99,0.01,by=-0.01),4)
