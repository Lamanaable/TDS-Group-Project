...
set.seed(1)
model_lasso <- cv.glmnet(X_train, Y_train, alpha = 1, family = "binomial", parallel=TRUE, standardize=FALSE)
...
LassoSub = function(k = 1, X=X_train, Y=Y_train, family = "binomial",
  ...
  model.sub = cv.glmnet(x = Xsub, y = Ysub, alpha = 1,
                        family = family, penalty.factor = penalty.factor, parallel=TRUE, standardize=FALSE)
