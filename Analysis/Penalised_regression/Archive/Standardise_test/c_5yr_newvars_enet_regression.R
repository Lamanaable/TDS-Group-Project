...
cvm = function(alpha) {
  model = cv.glmnet(x = X_train, y = Y_train, alpha = alpha, family="binomial", parallel=TRUE, standardize =TRUE) # Added standardize argument here
...
set.seed(1)
model_enet = cv.glmnet(x = X_train, y = Y_train, alpha = alpha.opt$minimum,family = "binomial", parallel=TRUE, standardize =TRUE)
...
EnetSub = function(k = 1, X=X_train, Y=Y_train, family = "binomial",
  ...
  model.sub = cv.glmnet(x = Xsub, y = Ysub, alpha = alpha.opt$minimum,
                        family = family, penalty.factor = penalty.factor, parallel=TRUE,standardize =TRUE)
...
enet.prop_min
saveRDS(enet.prop_min, "Outputs/c_enet_5yr_early_onset_stability_lambdamin.rds")
