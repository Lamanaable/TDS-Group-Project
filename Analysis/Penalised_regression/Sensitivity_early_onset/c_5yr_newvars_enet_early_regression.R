...
X_train <- read.csv("/rds/general/user/esb21/projects/hda_21-22/live/TDS/Group_2/TDS-Group-Project/Analysis/Penalised_regression/Sensitivity_early_onset/Data/CRC_5yr_train_early_onset_numeric.csv")
X_test <- read.csv("/rds/general/user/esb21/projects/hda_21-22/live/TDS/Group_2/TDS-Group-Project/Analysis/Penalised_regression/Sensitivity_early_onset/Data/CRC_5yr_test_early_onset_numeric.csv")
...
cvm = function(alpha) {
  model = cv.glmnet(x = X_train, y = Y_train, alpha = alpha, family="binomial", parallel=TRUE) # Added standardize argument here
...
set.seed(1)
model_enet = cv.glmnet(x = X_train, y = Y_train, alpha = alpha.opt$minimum,family = "binomial", parallel=TRUE) #Adding standardize=FALSE
...
enet.prop_min
saveRDS(enet.prop_min, "Outputs/c_enet_early_5yr_stability_lambdamin.rds")
