...
# Reading the data --------------------------------------------------------
CRC_ordered <-readRDS("/rds/general/user/esb21/projects/hda_21-22/live/TDS/Group_2/TDS-Group-Project/Data_exploration/Sensitivity_Data/CRC_rectum_only.rds")

stab<-readRDS("/rds/general/user/esb21/projects/hda_21-22/live/TDS/Group_2/TDS-Group-Project/Analysis/Penalised_regression/Stability_selection/5_year/Outputs_rectum/stab_selection_lasso_selprop.rds")
...
length(stab[stab>=stab_limit]) # only 8 selected variables with a proportion 82% -> matches the heatmap?
...
keep = c(dput(names(stab[stab>=stab_limit])))
length(keep)
...
library(pROC)
set.seed(2)
auc_ci<-ci.auc(Y_test, y_pred, boot.n=1000,method="bootstrap", boot.stratified=T) 
auc_ci
# AUC CI -> really bad!

pdf("Outputs_rectum/logistic_lasso_auc_stability_selection.pdf")
plot(roc_ROCR, colorize = TRUE)
title(main = "ROC curve: logistic regression rectum only - lasso stability selection")
abline(a = 0, b = 1)
