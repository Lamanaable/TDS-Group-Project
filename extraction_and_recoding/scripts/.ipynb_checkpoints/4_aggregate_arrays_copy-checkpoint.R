...
mydata <- readRDS("../outputs/ukb_recoded_2.rds")
choices <- read.xlsx("../parameters/parameters_2.xlsx")

# mydata <- ukb_recoded
# mydata <- mydata[sample(nrow(mydata), 100), ]

# path = '/rds/general/user/wq21/projects/hda_21-22/live/TDS/Group_2/TDS-Group-Project/extraction_and_recoding/parameters'
# setwd(path)
...
for (i in 1:nrow(choices)) {
  ...
  print(paste0('successed for line ',i))
}
cat("\n")
...
print('saving data')
saveRDS(mydata, "../outputs/ukb_final_2.rds")
