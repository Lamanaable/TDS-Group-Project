...
choices <- readRDS("../outputs/annot_2.rds")
mydata <- readRDS("../outputs/ukb_extracted_new_2.rds")
...
for (k in 1:ncol(mydata)) {
  ...
  if (!is.na(tmp_coding_id)) {
    ...
    coding <- read.table(paste0("../parameters/codings_2/codes_", tmp_coding_id, ".txt"), fill=TRUE, header = TRUE, stringsAsFactors = FALSE)
...
continuous_codings <- list.files(path = "../parameters/codings_2", pattern = "codes_field")
if (length(continuous_codings) > 0) {
  ...
  for (i in length(fields_to_recode)) {
    ...
    coding <- read.table(paste0("../parameters/codings_2/", continuous_codings[i]),
    ...
    for (j in ids) {
      ...
      recoded_data[, j] <- factor(recoded_data[, j], levels = tmp_coding$RecodedMeaning[sort.list(as.numeric(tmp_coding$RecodedValue))], 
                                  labels= tmp_coding$RecodedMeaning[sort.list(as.numeric(tmp_coding$RecodedValue))])
...
write.xlsx(choices, "../parameters/parameters_2.xlsx")

# Saving extracted dataset
saveRDS(mydata, "../outputs/ukb_recoded_2.rds")
